...
class App {
  constructor() {
    ...
    if (!storage.findUser('admin')) {
      storage.saveUser({
        username: 'admin',
        password: 'admin123',  // Store plain text password for initial admin creation
        role: 'admin',
        gender: 'other',
        email: 'admin@securiport.com',
        telephone: '123-456-7890',
        active: true // Ensure admin user is active
      });
      storage.saveFlightsAndSupervisors(['Brussel', 'Asky', 'Turkish Airline'], ['John Doe', 'Jane Smith', 'Bob Johnson']);
    }
  ...
  renderLogo() {
    const logo = document.createElement('img');
    logo.src = '/static/css/image/my-logo.png'; // Updated to my-logo.png as requested
    logo.alt = 'Securiport Logo';
    logo.className = 'mx-auto my-2 w-[200px] h-[50px] sm:w-[250px] sm:h-[62.5px] md:w-[300px] md:h-[75px] lg:w-[400px] lg:h-[100px]'; // Adjusted for logo dimensions
    this.container.appendChild(logo);
  }
  ...
  handleLogin(username, password) {
      ...
      .then(data => {
          if (data.success) {
              ...
              this.showModal('Login Failed', data.message, () => {});
          }
      })
      .catch(error => {
          console.error('Login error:', error);
          this.showModal('Login Error', 'An error occurred during login.', () => {});
      });
  }
  ...
  handleRegister(username, password, role, gender, email, telephone) {
      ...
      fetch('/register', {
          ...
          body: `username=${encodeURIComponent(username)}&password=${encodeURIComponent(password)}&role=${encodeURIComponent(role)}&gender=${encodeURIComponent(gender)}&email=${encodeURIComponent(email)}&telephone=${encodeURIComponent(telephone)}`,
      })
      .then(response => response.json())
      .then(data => {
          if (data.success) {
              this.showModal('Registration Successful', data.message, () => this.showLoginScreen());
          } else {
              this.showModal('Registration Failed', data.message, () => {});
          }
      })
      .catch(error => {
          console.error('Registration error:', error);
          this.showModal('Registration Error', 'An error occurred during registration.', () => {});
      });
  }
  ...
  handleReportSubmit(data) {
      ...
      .then(data => {
          if (data.success) {
              this.showModal('Success', 'Report submitted successfully!', () => this.showDashboard());
              // Reflect updates on Data Analyst dashboard
              if (this.currentUser.role === 'teamLead') {javascript
                this.updateDataAnalystTable();
            }
        } else {
            this.showModal('Error', 'Failed to submit the report. Please try again.', () => {});
        }
    })
    .catch(error => {
        console.error('Report submission error:', error);
        this.showModal('Submission Error', 'An error occurred during report submission.', () => {});
    });
}
...
handleReportUpdate(id, updatedData) {
  ...
  if (this.currentUser.role === 'teamLead' && report.submittedBy === this.currentUser.username && storage.isUpdateActivated(this.currentUser.username, selectedDate)) {
    ...
    .catch(error => {
        ...
        this.showModal('Update Error', 'An error occurred during report update.', () => {});
    });
  } else if (this.currentUser.role === 'dataAnalyst') {
    const iicsInfant = Number(updatedData.iicsInfant) || 0;
    const iicsAdult = Number(updatedData.iicsAdult) || 0;
    ...
    fetch(`/reports/${id}`, {
        ...
        body: JSON.stringify({...updatedData, iicsTotalDifference, giaTotalDifference}),
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
...
handleReportVerification(id, { iicsInfant, iicsAdult, iicsTotal, giaInfant, giaAdult, giaTotal, iicsTotalDifference, giaTotalDifference }) {
  ...
  fetch(`/reports/${id}/verify`, {
      method: 'PUT',
      headers: {
          'Content-Type': 'application/json',
      },
      body: JSON.stringify({verifiedBy: this.currentUser.username}),
  })
  .then(response => response.json())
  .then(data => {
      if (data.success) {
          ...
          this.showModal('Error', 'Failed to verify the report. Please try again.', () => {});
      }
  })
  .catch(error => {
      console.error('Report verification error:', error);
      this.showModal('Verification Error', 'An error occurred during report verification.', () => {});
  });
}
...
showUserManagementModal(searchQuery = '') {
  ...
  modal.querySelectorAll('.deactivate-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const username = btn.dataset.username;
      const action = storage.findUserByUsername(username).active ? 'deactivate' : 'activate';
      
      fetch(`/users/${username}?action=${action}`, {
          method: 'PUT',
      })
      .then(response => response.json())
      .then(data => {
          if (data.success) {
              ...
              this.showModal('Success', `User ${username} ${action === 'activate' ? 'activated' : 'deactivated'} successfully!`, () => this.showUserManagementModal(searchQuery));
          } else {
              this.showModal('Error', data.message, () => {});
          }
      })
      .catch(error => {
          console.error('User activation/deactivation error:', error);
          this.showModal('Action Error', 'An error occurred during user activation/deactivation.', () => {});
      });
    });
  });
    ...
    btn.addEventListener('click', () => {
      ...
      if (confirm(`Are you sure you want to delete ${username}?`)) {
          ...
          .catch(error => {
              ...
              this.showModal('Deletion Error', 'An error occurred during user deletion.', () => {});
          });
      }
    });
  });
  ...
  modal.querySelectorAll('.change-password-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const newPassword = prompt('Enter new password (min 8 chars, 1 uppercase, 1 lowercase, 1 number, 1 special char):');
      if (newPassword && /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/.test(newPassword)) {
          fetch(`/users/${username}?action=update_password`, {
              method: 'PUT',
              headers: {
                  'Content-Type': 'application/json',
              },
          ...
          .catch(error => {
              ...
              this.showModal('Update Error', 'An error occurred during password update.', () => {});
          });
      } else {
        this.showModal('Error', 'Invalid password format.', () => {});
      }
...
showFlightsSupervisorsModal() {
  ...
  modal.querySelector('#saveFlightsSupervisors').addEventListener('click', () => {
    ...
    fetch('/flights_supervisors', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({flights: flightsInput, supervisors: supervisorsInput}),
    })
    .then(response => response.json())
    .then(data => {
        if (data.success) {
    ...
    .catch(error => {
        console.error('Flights/Supervisors update error:', error);
        this.showModal('Update Error', 'An error occurred during flights/supervisors update.', () => {});
    });
  });
}
